...
if [ ! -f "/var/www/html/wp-config.php" ]; then
	...
	wp core install --allow-root --admin_email=$WP_ADMIN_EMAIL --url=$DOMAIN_NAME --title="Ismo's site" --admin_user=$WP_ADMIN_USER --prompt=admin_password --skip-email < $WORDPRESS_ADMIN_PASSWORD_FILE 
fi
